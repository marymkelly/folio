...
module.exports = plugin(({ addUtilities, matchUtilities, theme, e }) => {
		...
		return Object.keys(theme).reduce((acc, key) => {
			...
			if (typeof value === "object") {
				return Object.assign(
					acc,
					flattenTheme(value, { prefix: `${opts?.prefix ?? ""}${key}${opts?.separator ?? "-"}`, separator: opts?.separator ?? "-" })
				);
	...
	const dynamicUtility = {
		"text-stroke": (value) => {
			if (Number.isInteger(Number.parseInt(value[0])) || ["thin", "medium", "thick"].includes(value)) {
				return { "text-stroke-width": value, "-webkit-text-stroke-width": value };
			}
